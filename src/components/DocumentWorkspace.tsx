...
export const DocumentWorkspace = ({
  ...
  const generateDocument = async (type: DocType) => {
    ...
    try {
      ...
      const prompt = `You are an expert legal drafter for ${country}.
Generate a professional ${type.toUpperCase()} draft for a ${caseType} case based on the following conversation context.
Use standard legal formatting and language.

Conversation Context:
${chatContext}

Return ONLY the document text. Do not include markdown formatting like \`\`\` or extra conversational text.`;
  ...
  const scanDocument = async () => {
    ...
    try {
      const ai = getGeminiClient();
      const prompt = `You are an expert legal reviewer for ${country}.
Review the following ${activeDoc || 'legal document'} for a ${caseType} case.
Analyze it for legal correctness, missing elements, and potential issues.
...
${documentText}`;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              verdict: { type: Type.STRING, description: "One of: VALID, NEEDS_CHANGES, INVALID" },
              summary: { type: Type.STRING, description: "A brief summary of the review" },
              issues: {
                type: Type.ARRAY,
                items: {
                  type: Type.OBJECT,
                  properties: {
                    ...
                    location: { type: Type.STRING, description: "The specific text or section with the issue" },
            ...
            required: ["overallScore", "verdict", "summary", "issues", "strengths", "missingElements"]
          }
        }
      });
      ...
      if (result) {
        ...
        if (verdict === 'VALID') {
          ...
          toast.warning(`Found ${result.issues?.length || 0} issues that need attention`);
        } else {
  ...
  const editWithAI = async () => {
    ...
    try {
      const ai = getGeminiClient();
      const prompt = `You are an expert legal drafter for ${country}.
Edit and improve the following ${activeDoc || 'legal document'} for a ${caseType} case.
Fix any legal issues, improve the language, and ensure it meets professional standards.

Document Text:
${documentText}

Return ONLY the updated document text. Do not include markdown formatting like \`\`\` or extra conversational text.`;
